...
describe('Profiles base64 size enforcement (e2e)', () => {
  ...
  it('oversized base64 is rejected', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .send({
        ...
        pdfBase64: oversizedBase64,
      })
      .expect(413);
  });

  it('normal base64 is accepted', async () => {
    const normalBuffer = Buffer.from('%PDF-1.4\nsmall pdf content');
    const normalBase64 = normalBuffer.toString('base64');

    await request(app.getHttpServer())
      .post('/api/profiles/me/send-cv-email')
      .send({
        email: 'recipient@example.com',
        pdfBase64: normalBase64,
      })
      .expect(201);
  });
});
